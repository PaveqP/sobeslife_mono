...
This project has a knowledge graph at `graphify-out/graph.json`.

- **Before answering architecture or codebase questions**, check the graph: `/graphify query "<question>"`
- **After significant code changes**, rebuild: `/graphify . --update`
- Graph outputs: `graphify-out/graph.json` (GraphRAG), `graphify-out/graph.html` (interactive), `graphify-out/GRAPH_REPORT.md` (summary)
...
### RuFlo — ростер монорепо (this project)

- Роли и папки: `.claude/ruflo-sobeslife-roster.json`
- На **каждый** промпт хук `UserPromptSubmit` вызывает `router.js` → в контекст попадают primary agent, `[Parallel roster]` и `[Spawn hint]` (см. вывод хука)
- Исполнение: координатор распределяет работу; параллельно — Task-субагенты или MCP `agent_spawn` / `task_orchestrate` (`@claude-flow/cli`; пакет `ruflo` — обёртка над тем же CLI)
