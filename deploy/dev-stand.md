...
| **`column "dirty" does not exist`** / **`null value in column "name"`** в `schema_migrations` | Таблица не от golang-migrate (чужие колонки, часто `name NOT NULL`). Приложение при старте чинит типовой случай (`DROP name`, `ADD dirty`). Вручную: `... -c 'ALTER TABLE IF EXISTS public.schema_migrations DROP COLUMN IF EXISTS name; ALTER TABLE IF EXISTS public.schema_migrations ADD COLUMN IF NOT EXISTS dirty boolean NOT NULL DEFAULT false;'` |
